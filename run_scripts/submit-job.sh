...
for it in {3..3}; do
    ...
    for model_size in "${model_sizes[@]}"; do
        ...
        #bash config-n-run.sh -i $it -c 0 -h 16 -m $model_size -H $H -F $F -N $N -L $L -U $U -S $S -K $K -M $M -B $B -I $I -P $P -T $T -D $D
        bash config-n-run.sh -i $it -c 4 -h 16 -m $model_size -H $H -F $F -N $N -L $L -U $U -S $S -K $K -M $M -B $B -I $I -P $P -T $T -D $D
        #bash config-n-run.sh -i $it -c 4 -h 16 -m $model_size -H $H -F $F -N $N -L $L -U $U -S $S -K $K -M $M -B $B -I $I -P 0 -T $T -D $D
        # Checkpoint with TorchSnapshot approach.
        #bash config-n-run.sh -i $it -c 5 -h 0 -m $model_size -H $H -F $F -N $N -L $L -U $U -S $S -K $K -M $M -B $B -I $I -P $P -T $T -D $D
    done
done
############### Run for diff model sizes.
